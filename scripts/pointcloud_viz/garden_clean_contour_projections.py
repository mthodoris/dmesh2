...
grid_res = 300
out_dir = "data/3d/garden_clean_heatmaps"

# mask threshold scaled to this cloud's point spacing (median NN ~0.13)
MASK_DIST = 0.6


def contour_projection(u, v, w, u_label, v_label, w_label, out_path, mask_dist=MASK_DIST):
...
# 1) XY plane (top-down), colored by Z
contour_projection(x, y, z, "X", "Y", "Z", f"{out_dir}/garden_clean_heatmap_xy_by_z.png")

# 2) XZ plane (front view), colored by Y
contour_projection(x, z, y, "X", "Z", "Y", f"{out_dir}/garden_clean_heatmap_xz_by_y.png")

# 3) YZ plane (side view), colored by X
contour_projection(y, z, x, "Y", "Z", "X", f"{out_dir}/garden_clean_heatmap_yz_by_x.png")
