...
ALPHA_FACTOR = 4.0  # max allowed triangle edge, as a multiple of the local median nearest-neighbor distance
...
def alpha_filtered_triangulation(px, py, alpha_factor=ALPHA_FACTOR):
    ...
    real gaps in the slice (e.g. between ribs, or across disconnected parts)."""
...
for axis_name, info in axes.items():
    ...
    for i in range(n_bins):
        ...
        if triang is not None:
            axs[1].triplot(triang, color=color, linewidth=0.5)
            axs[1].scatter(px, py, s=2, c="black", alpha=0.4, zorder=3)
